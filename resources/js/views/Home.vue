    ...
    <section class="py-8 bg-white dark:bg-gray-800 border-b dark:border-gray-700">
      <div class="container mx-auto px-4">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
          <div>
            <div class="text-3xl font-bold text-primary-600 dark:text-primary-400">{{ stats.users }}</div>
            <div class="text-gray-600 dark:text-gray-400">Developers</div>
          </div>
          <div>
            <div class="text-3xl font-bold text-primary-600 dark:text-primary-400">{{ stats.posts }}</div>
            <div class="text-gray-600 dark:text-gray-400">Posts</div>
          </div>
          <div>
            <div class="text-3xl font-bold text-primary-600 dark:text-primary-400">{{ stats.jobs }}</div>
            <div class="text-gray-600 dark:text-gray-400">Jobs</div>
          </div>
          <div>
            <div class="text-3xl font-bold text-primary-600 dark:text-primary-400">{{ stats.events }}</div>
        ...
        <div class="grid lg:grid-cols-3 gap-8">
          ...
          <div class="lg:col-span-2">
            <div class="flex items-center justify-between mb-6">
              ...
              <router-link to="/community" class="text-primary-600 dark:text-primary-400 hover:underline">
                View All
              </router-link>
            </div>

            <div v-if="loading" class="space-y-4">
              <div v-for="i in 3" :key="i" class="bg-white dark:bg-gray-800 rounded-lg p-6 animate-pulse">
          ...
          <div class="space-y-8">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg p-6">
              ...
              <router-link to="/events" class="block mt-4 text-center text-primary-600 dark:text-primary-400 hover:underline">
                View All Events
              </router-link>
            </div>
...
onMounted(async () => {
  try {
    const [statsRes, postsRes, categoriesRes, eventsRes, usersRes] = await Promise.all([
      axios.get('/api/v1/stats'),
      axios.get('/api/v1/posts?per_page=5'),
      axios.get('/api/v1/categories'),
      axios.get('/api/v1/events?upcoming=true&per_page=3'),
      axios.get('/api/v1/users/leaderboard?per_page=5'),
    ])
